  ...
  g_cpu_duration <- ggplot(cpu_data, aes(x = factor(dimension), y = encode_duration_s *100, fill = encodertype)) +
    geom_col(position = position_dodge(width = 0.8), width = 0.7) +
    scale_fill_brewer(palette = "Dark2") +
    labs(
      title = "CPU Time",
      y = "Time Spendng (seconds)",
      color = "Encoder Type"
    ) +
  ...
  g_cpu_duration <- ggplot(
    cpu_data %>% mutate(cpu_time_sec = if_else(cpu_time_sec <= 0, 1e-6, cpu_time_sec)),
    aes(x = factor(dimension), y = cpu_time_sec, fill = encodertype)
  ) +
    geom_col(position = position_dodge(width = 0.8), width = 0.7) +
    geom_text(
      aes(label = sprintf("%.2f", cpu_time_sec)),
      position = position_dodge(width = 0.8),
      vjust = -0.25, size = 2.8, angle = 90
    ) +
    scale_fill_brewer(palette = "Dark2") +
    ...
    labs(
      title = "CPU Time",
      x = "Feature Dimension",
      y = "Time Spending (seconds, log10 scale)",
      fill = "Encoder Type"
    ) +
    coord_cartesian(clip = "off") +
    theme_minimal() +
    theme(plot.margin = margin(10, 15, 20, 10))
